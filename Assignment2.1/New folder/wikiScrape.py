...
def startPage(url):
    ...
    processedActorsLink.append(url)
    ...
    for g in gross:
        if i == len(gross)-1:
            try:
                ...
                gross_val = gross_val.replace(",", "")
                
                
                try:
                    dot = gross_val.find(".")
                    space = gross_val.find(" ")
                except:
                    pass
                ...
                if dot != -1:
                    if space != -1:
                        diff = space - dot
                try:
                    mil = gross_val.split(" ")[1]
                    if mil == "million":
                        mil_flag = True
                except: 
                    pass
                #print(gross_val)
                #print(diff, dot, space)
                gross_val = re.sub("\D", "", gross_val)
                #print(gross_val)
                if diff == -2 and mil_flag == False:
                    break
                
                if diff == 3:
                    gross_val = gross_val + "0000"
                if diff == 2:
                    gross_val = gross_val + "00000"
                if diff == 4:
                    gross_val = gross_val + "000000"
                
                if mil_flag == True and dot == -1:
                    gross_val = gross_val + "000000"
                #7237794334941
                if gross_val > 1000000000000:
                    gross_val = gross_val / 10000000
                #print(gross_val)
            except:
                pass
        i = i + 1
